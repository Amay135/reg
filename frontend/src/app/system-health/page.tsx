...
function StatusDot({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-block w-2.5 h-2.5 rounded-full ${colors[status] || "bg-zinc-400"} animate-pulse`}
    />
  );
}
...
function StatusLabel({ status }: { status: string }) {
  ...
  return (
    <span className={`text-xs font-semibold ${colors[status] || "text-zinc-400"}`}>
...
export default function SystemHealthPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [svc, lat, err] = await Promise.all([
          fetchServices(),
          fetchLatency(),
          fetchErrors(20),
        ]);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        ...
        <span className="text-sm font-mono text-zinc-500">{Math.round(ms)}ms</span>
      </div>
